...
-- message are derived from this constant to keep them in sync.
...
function Atr_FullScanFrameIdle()
    ...
    if (gAtr_FullScanState == ATR_FS_STARTED) then
        ...
        if (not gAtr_FullScanTimedOut and (time() - gAtr_FullScanStart) > FULL_SCAN_TIMEOUT_SECONDS) then
            ...
            local timeoutMinutes = FULL_SCAN_TIMEOUT_SECONDS / 60;
            gAtr_FullScanStopReason = string.format(
                "Stopped after %g %s: server did not return all auctions",
                timeoutMinutes, timeoutMinutes == 1 and "minute" or "minutes"
            );
